...
class StackWidget : AppWidgetProvider() {
    ...
    companion object {
        private const val TOAST_ACTION = "com.example.githubuserapi.TOAST_ACTION"
        const val EXTRA_ITEM = "com.example.githubuserapi.EXTRA_ITEM"
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action != null) {
            if (intent.action == TOAST_ACTION) {
                ...
                Toast.makeText(context, "User $viewIndex", Toast.LENGTH_SHORT).show()
            }
        }
    }
}